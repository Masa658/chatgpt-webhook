...
Q: 切削加工とは？
A: 切削加工は、金属や樹脂の材料を工作機械を使って削り出す加工技術です。加工プログラムと材料の用意で、最小ロット1個・最短当日完成の加工が可能です。そのため、多品種小ロットの生産や試作に適しています。切削加工は角物形状を削るフライス加工と、丸物形状を削る旋盤加工に大別されます。これらの組み合わせによって幅広い形状の削り出しが可能です。

Q: 切削加工のデメリットや注意点は？
A: 切削加工には、単品から加工できるというメリットの反面、量産向きの成形や鋳造などの加工方法と比べると単価は高めです。1個のみの加工が最も単価が高く、数量が増えると単価は低くなる傾向があります。これは加工を開始するために必要なプログラム作成や仕込みの時間が按分されるためです。また、大きな材料から削る必要のある形状は、製品の体積や外形から想定される材料費よりも高額になることがあります。

Q: 切削加工の依頼方法は？
A: 図面があり、材質などの条件が決まっている場合は、見積もり依頼フォームからお問い合わせください。図面がない場合や、加工の相談からスタートしたい場合は相談依頼フォームからお問い合わせください。お見積り・ご相談は無料です。

Q: 見積もり依頼ページはどこですか？
A: https://yumoparts.com/contact

Q: メールで見積り依頼したいです。
A: こちらのメールアドレスからお問い合わせください。yumoparts@yumoto.jp

Q: 電話番号は？
A: 06-6976-3366です。お電話でのお見積りの回答はできかねますので、その点ご了承ください。

Q: 切削加工の対応加工は？
A: マシニング加工（3軸）、マシニング加工（5軸）、NC旋盤加工、切断加工、追加工です。難形状の加工にも対応しています。

Q: 切削加工の最大ワークサイズは？
A: 樹脂全般は1000 × 500 × 200[mm]（マシニング加工）、Φ300 × 500L（旋盤加工）です。アルミニウム合金は700 × 400 × 150[mm]（マシニング加工）、Φ300 × 500L（旋盤加工）です。その他金属材質は320 × 320 × 200[mm]（マシニング加工）、Φ300 × 500L（旋盤加工）です。

Q: 切削加工後の仕上げにも対応できますか？
A: はい、できます。対応表面処理はアルマイト、アロジン処理、無電解ニッケル、硬質クロム、三価クロメート、めっき、塗装、テフロンコーティング、蒸着です。対応後処理はバフ研磨、バレル研磨、ブラスト、鏡面磨き、手磨きです。
